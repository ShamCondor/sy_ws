...
private:
    ...
    // 准备订阅ModelRanger的Callback, 但发现频率仍受限，待查
    // 激光回调
    static bool LaserUpdate( Stg::ModelRanger *ranger, StageNode *node)
    {
//        std::cout << ros::Time::now() << std::endl;
        node->LaserCallback( ranger);
        return false;
    }
    void LaserCallback( Stg::ModelRanger *ranger)
    {
//        std::cout << sim_time << std::endl;
        broadcastLaser_sub();
    }

    static bool OdomUpdate( Stg::ModelPosition *position, StageNode *node)
    {
//        std::cout << "odom:" << ros::Time::now() << std::endl;
        node->OdomCallback( position);
        return false;
    }
    void OdomCallback( Stg::ModelPosition *position)
    {
//        std::cout << sim_time << std::endl;
        broadcastOdom_sub();
    }
...
public:
    ...
    // 新增单独的激光广播
    double getRangerZeroFrequence();
    void broadcastLaser_sub();
    void broadcastLaser();

    // 新增单独的码盘广播
    void broadcastOdom_sub();
};
...
StageNode::StageNode(int argc, char** argv, bool gui, const char* fname, bool use_model_names)
{
    ...
    double t;
    ros::NodeHandle localn("~");//privste node handle
    ...
    struct stat s;
    if(stat(fname, &s) != 0)//获得fname目录的属性
    {
    ...
    // 单独注册激光的 callback
    laser = lasermodels[ 0];
    laser->AddCallback( Stg::Model::CB_UPDATE, (Stg::model_callback_t)LaserUpdate, this);
    laser->Subscribe();//subscribe to a model's data

    // 单独注册码盘的 callback
    position = positionmodels[ 0];
    position->AddCallback( Stg::Model::CB_UPDATE, (Stg::model_callback_t)OdomUpdate, this);
    position->Subscribe();
}
...
//    n_.setParam("/use_sim_time", true);
    ...
    n_.getParam( "/use_sim_time", use_sim_time);
	...
	// TODO - print the topic names nicely as well
        ...
        new_robot->cmdvel_sub = n_.subscribe<geometry_msgs::Twist>(mapName(CMD_VEL, r, static_cast<Stg::Model*>(new_robot->positionmodel)), 10, boost::bind(&StageNode::cmdvelReceived, this, r, _1));//两个参数r和_1,this指向前面那个地址变量
        ...
        for (size_t s = 0; s < robotmodel->lasermodels.size(); ++s)
        {
            ...
            if( sensor.ranges.size() )
            {
                ...
                if (robotmodel->lasermodels.size() > 1)
                    msg.header.frame_id = mapName("laser_frame", r, s, static_cast<Stg::Model*>(robotmodel->positionmodel));
                else
                    msg.header.frame_id = mapName("laser_frame", r, static_cast<Stg::Model*>(robotmodel->positionmodel));
            ...
            if (robotmodel->lasermodels.size() > 1)
                tf.sendTransform(tf::StampedTransform(txLaser, sim_time,
                                                      ...
                                                      mapName("laser_frame", r, s, static_cast<Stg::Model*>(robotmodel->positionmodel))));
            else
                tf.sendTransform(tf::StampedTransform(txLaser, sim_time,
                                                      mapName("base_link", r, static_cast<Stg::Model*>(robotmodel->positionmodel)),
                                                      mapName("laser_frame", r, static_cast<Stg::Model*>(robotmodel->positionmodel))));
        }
*/
...
//                                              sim_time,
                                              use_sim_time ? sim_time : ros::Time::now(),
...
/* odom 不在这里发布了
        ...
        odom_msg.child_frame_id  = mapName("base_link", r, static_cast<Stg::Model*>(robotmodel->positionmodel));

//        odom_msg.header.stamp = sim_time;
        odom_msg.header.stamp = use_sim_time ? sim_time : ros::Time::now();
        ...
        tf.sendTransform(tf::StampedTransform(txOdom,
//                                              sim_time,
                                              use_sim_time ? sim_time : ros::Time::now(),
...
*/
        ...
        //he base_pose_ground_truth topic always provides a perfect, globally referenced pose for the robot in the simulation, independent of .world file settings. The base_pose_ground_truth data is intended for testing purposes; it should not be used in robot control loops (because it's unrealistic).
...
//        ground_truth_msg.header.stamp = sim_time;
        ground_truth_msg.header.stamp = use_sim_time ? sim_time : ros::Time::now();
...
//                image_msg.header.stamp = sim_time;
                image_msg.header.stamp = use_sim_time ? sim_time : ros::Time::now();
...
//                depth_msg.header.stamp = sim_time;
                depth_msg.header.stamp = use_sim_time ? sim_time : ros::Time::now();
                    ...
                    || (robotmodel->depth_pubs[s].getNumSubscribers()>0 && cameramodel->FrameDepth()))
            {
                ...
                if (robotmodel->cameramodels.size() > 1)
                    tf.sendTransform(tf::StampedTransform(tr,
//                                                          sim_time,
                                                          use_sim_time ? sim_time : ros::Time::now(),
                                                          mapName("base_link", r, static_cast<Stg::Model*>(robotmodel->positionmodel)),
...
//                camera_msg.header.stamp = sim_time;
                camera_msg.header.stamp = use_sim_time ? sim_time : ros::Time::now();
...
//    clock_msg.clock = sim_time;
    clock_msg.clock = use_sim_time ? sim_time : ros::Time::now();

    this->clock_pub_.publish(clock_msg);
}


double StageNode::getRangerZeroFrequence()
{
    // 机器人
    StageRobot const * robotmodel = this->robotmodels_[ 0];

    // Ranger 设备
    Stg::ModelRanger const* lasermodel = robotmodel->lasermodels[ 0];

    // 取得激光更新时间间隔
//    std::cout << "lasermodel->GetUpdateInterval(): " << lasermodel->GetUpdateInterval() << std::endl;

    return 1e6 / lasermodel->GetUpdateInterval();
}

void StageNode::broadcastLaser_sub()
{
//    printf( "broadcastLaser_sub...\n");
...
//                msg.header.stamp = sim_time;
                msg.header.stamp = use_sim_time ? sim_time : ros::Time::now();
            ...
            if (robotmodel->lasermodels.size() > 1)
                tf.sendTransform(tf::StampedTransform(txLaser,
//                                                      sim_time,
                                                      use_sim_time ? sim_time : ros::Time::now(),
                                                      mapName("base_link", r, static_cast<Stg::Model*>(robotmodel->positionmodel)),
                                                      mapName("laser_frame", r, s, static_cast<Stg::Model*>(robotmodel->positionmodel))));
...
void StageNode::broadcastOdom_sub()
{
    size_t r = 0;
    ...
    odom_msg.twist.twist.linear.y = v.y;
    odom_msg.twist.twist.angular.z = v.a;
    //ROS_INFO("odom.pose.x=%.4lf",odom_msg.pose.pose.position.x);
...
void StageNode::broadcastLaser()
{
    double freqLaser = getRangerZeroFrequence();
    std::cout << "freqLaser:" << freqLaser << std::endl;

    ros::Rate loop_rate( freqLaser);
    while (ros::ok())
    {
        broadcastLaser_sub();
...
int main(int argc, char** argv)
{
    ...
    {
        if(!strcmp(argv[i], "-g"))//字符串1＝字符串2，返回值＝0；
            gui = false;
        if(!strcmp(argv[i], "-u"))
            use_model_names = true;
    }

    ROS_INFO("#############%d#####%s####%s",argc,argv[0],argv[1]);
    //argc = 2  argv[0]=/home/ares/sy_ws/devel/lib/stage_ros/stageros
    //          argv[1]=/home/ares/maps/ares.world
...
// 不再使用独立线程发布激光
//    boost::thread threadRangerPub = boost::thread( boost::bind( &StageNode::broadcastLaser, &sn));
